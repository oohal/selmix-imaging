...
scan_points = [100,        100]; % number of points to scan in the area
...
for i = start_at:length(path)
    ...
    for k = 1:2:scan_points(1)
        mag_pic(:,k) = flipud(mag_pic(:,k));
        phase_pic(:,k) = flipud(phase_pic(:,k));
    end
    
    for k = 1:scan_points(2)
        mag_pic(k,:)   = fliplr(mag_pic(k,:));
        phase_pic(k,:) = fliplr(phase_pic(k,:));
    end
