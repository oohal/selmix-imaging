...
% selmixpower_v - vector version of selmixpower
% C     The feedback strength parameter
% phi   vector of round trip phase changes without feedback
% alpha linewidth enhancement factor of the laser
% m     internal hystersis variable, if unsepecifed 0 is assumed
function [power, m] = selmixpower_v (C, phi, alpha, m) % Power level at a sample in time
    if ~exist('m', 'var');
        m = 0;
    end
    ...
    for i = 1:length(phi)
        [power(i), m] = selmixpower_m(C, phi(i), alpha, m);
    end
end

function [power, m_out] = selmixpower_m (C, phi0, alpha, m_in) % Power level at a sample in time
    if (C <= 1.0)
        [phimin, phimax] = boundsweak (C, phi0);
        m_out = m_in;
    else
        [phimin, phimax, m_out] = boundsstrong (C, phi0, alpha, m_in);
    end
...
function [phimin, phimax, m_out] = boundsstrong (C, phi0, alpha, m) % Find search region when C > = 1
